...
const AccountabilityChain = ({ complaintId, lat, lng, issueType, location }) => {
  ...
  useEffect(() => {
    ...
    fetchChain();
  }, [complaintId, lat, lng, issueType, location]);

  if (loading) {
    return (
      <div className="bg-gradient-to-br from-slate-50 to-indigo-50 rounded-2xl p-6 border border-slate-200 animate-pulse">
    ...
    return (
      <div className="bg-red-50 rounded-2xl p-5 border border-red-100 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-red-600 mt-1 opacity-80">Could not determine responsible authorities for this location.</p>
        </div>
      </div>
    );
  }
